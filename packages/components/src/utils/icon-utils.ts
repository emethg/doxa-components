import { DOXA_ICONS_CDN_URL } from "@/constants";
...
const getIconUrl = (
  ...
  if (iconLibrary === "lucide") {
    return `${DOXA_ICONS_CDN_URL}/lucide/v0.545.0/${icon}.svg`;
  }
  ...
  if (isBrandsIcon(icon)) {
    return `${DOXA_ICONS_CDN_URL}/v7.1.0/brands/${icon}.svg`;
  }

  const type = iconType ?? "regular";

  return `${DOXA_ICONS_CDN_URL}/v7.1.0/${type}/${icon}.svg`;
};
...
const getLanguageIconUrl = (language: string): string | null => {
  ...
  return `${DOXA_ICONS_CDN_URL}/devicon/${iconKey}.svg`;
};
