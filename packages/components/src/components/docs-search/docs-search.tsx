...
export function DocsSearch({ items, linkComponent: L = "a" }: DocsSearchProps) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: `query` drives the reset; `setActive` is stable.
  useEffect(() => {
    setActive(0);
  }, [query]);
      ...
      {open &&
        ...
        createPortal(
          <div
            ...
            role="dialog"
          >
            {/* Backdrop as a real button for click-to-close; Escape is handled
                by the global keydown listener above. */}
            <button
